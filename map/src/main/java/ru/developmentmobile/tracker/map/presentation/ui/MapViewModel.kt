...
class MapViewModel(
        ...
        when (mapUiEvents) {
            ...
            is MapUiEvents.LoadSectionData -> {
                ...
                when (cachedData.section) {
                    Section.TRACKS -> loadTracks()
                    Section.LOCATIONS -> loadLocations(true)
                    Section.BEACON -> loadBeacons()
                }
            }
    ...
    private fun loadTracks() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            delay(1000)
            if (cachedData.section == Section.TRACKS) {
    ...
    private fun loadLocations(needToLoadMarkers: Boolean) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            delay(1000)
            if (cachedData.section == Section.LOCATIONS) {
                postValue(MapUiModel.ShowProgressSectionData(cachedData.section, false))
                delay(50)
                postValue(MapUiModel.LoadSectionData())
            }
    ...
    private fun loadBeacons() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            delay(1000)
            if (cachedData.section == Section.BEACON) {
                postValue(MapUiModel.ShowProgressSectionData(cachedData.section, false))
                delay(50)
                postValue(MapUiModel.LoadSectionData())
            }
    ...
    private fun observeBeacon() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            while (cachedData.observeBeaconSwitch) {
                //spy the beacon
                when (val request = mapBeaconInteractor.getBeacon(1)) {
                    is Success -> {
                ...
                postValue(MapUiModel.UpdateBeacon())
                delay(6000)
            }
